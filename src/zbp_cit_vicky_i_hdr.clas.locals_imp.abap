...
CLASS lhc_SalesOrderHdr IMPLEMENTATION.
  ...
  METHOD get_instance_features.
    ...
    READ ENTITIES OF ZCIT_VICKY_I_HDR IN LOCAL MODE
      ENTITY SalesOrderHdr
        FIELDS ( FileStatus ExcelAttachment )
    ...
    LOOP AT lt_hdr INTO DATA(ls_hdr).
      ...
      IF ls_hdr-FileStatus = 'File Selected'.
        ls_result-%action-uploadExcelData = if_abap_behv=>fc-o-enabled.
      ELSE.
        ls_result-%action-uploadExcelData = if_abap_behv=>fc-o-disabled.
      ENDIF.
      IF ls_hdr-ExcelAttachment IS INITIAL.
        ls_result-%action-DownloadExcel = if_abap_behv=>fc-o-enabled.
      ELSE.
        ls_result-%action-DownloadExcel = if_abap_behv=>fc-o-disabled.
      ENDIF.
      APPEND ls_result TO result.
    ENDLOOP.
  ENDMETHOD.
  ...
  METHOD uploadExcelData.
    ...
    DATA lv_serial TYPE int2 VALUE 1.
    ...
    LOOP AT keys INTO ls_key.
      ...
      LOOP AT lt_raw INTO DATA(ls_row).
        ...
        " REQUIRED FIX: Pass the Draft status to the item to avoid short dumps
        ...
        " =================================================================
        " NEW SAFETY NET: Prevent Unit/Quantity errors from crashing the app
        " =================================================================
        DATA lv_raw_qty  TYPE string.
        DATA lv_raw_unit TYPE string.

        " 1. Auto-Swap: Check if Quantity (col4) and Unit (col5) were accidentally swapped
        IF ls_row-col5 CO ' 0123456789.,'. " If column 5 is just numbers
        ...
        " 2. Clean up Quantity formatting
        ...
        " 3. Clean up Unit (SAP strictly requires uppercase, e.g., 'ea' -> 'EA')
        ls_target-Quantityunits = to_upper( lv_raw_unit ).

        " 4. Auto-Default: If unit is blank or still a number, default to 'EA' (Each)
    ...
    " Re-read the header to return the updated values
    READ ENTITIES OF ZCIT_VICKY_I_HDR IN LOCAL MODE
      ENTITY SalesOrderHdr ALL FIELDS WITH CORRESPONDING #( keys )
